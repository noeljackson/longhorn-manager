...
func TestCodewireDirectVolumeDeploymentInputs(t *testing.T) {
	...
	for _, volumeMount := range pluginContainer.VolumeMounts {
		if volumeMount.Name == "codewire-direct-volume-state" && volumeMount.MountPath == codewireDirectVolumeStateDir {
			foundMount = true
		}
	}
	if !foundMount {
	...
	for _, volume := range plugin.daemonSet.Spec.Template.Spec.Volumes {
		if volume.Name == "codewire-direct-volume-state" && volume.HostPath != nil && volume.HostPath.Path == codewireDirectVolumeStateDir && volume.HostPath.Type != nil && *volume.HostPath.Type == corev1.HostPathDirectoryOrCreate {
			foundVolume = true
		}
	}
	if !foundVolume {
