...
Before registration, the node plugin verifies that the publishing Pod is on
the local node, references this PVC, and owns the exact kubelet CSI target
path. If the Pod requests an `fsGroup`, that bounded ownership metadata is
forwarded for the guest mount; it does not select encryption or release a key.
...
No key, protection profile, encryption parameters, or plaintext are present in
