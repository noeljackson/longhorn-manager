...
const (
	codewireKataDirectVolumeParameter = "codewireKataDirectVolume"
	codewireStorageKeyIDAnnotation    = "codewire.sh/confidential-storage-key-id"

	csiPVCNameKey      = "csi.storage.k8s.io/pvc/name"
	csiPVCNamespaceKey = "csi.storage.k8s.io/pvc/namespace"

	codewireStorageEncryptionKey = "io.codewire.storage.encryption"
	codewireStorageSourceKey     = "io.codewire.storage.source"
	codewireStorageKeyURIKey     = "io.codewire.storage.key-uri"
	codewireStorageFilesystemKey = "io.codewire.storage.filesystem"
	codewireStorageGrowKey       = "io.codewire.storage.grow"

	codewireStorageKeyURIPrefix = "kbs:///default/codewire-workspace-luks/"

	codewireDirectVolumeStateDir = "/var/lib/longhorn/codewire-csi-direct-volumes"
	kataRuntimePath              = "/opt/kata/bin/kata-runtime"
	nsMounterPath                = "/usr/local/sbin/nsmounter"

	codewireDirectVolumeRestartRequired = "confidential direct volume expansion requires pod restart after offline expansion"
)

type codewireDirectVolumeMountInfo struct {
...
type codewireDirectVolumeState struct {
...
type codewireKataRuntime interface {
	Add(ctx context.Context, targetPath string, mountInfo codewireDirectVolumeMountInfo) error
	Remove(ctx context.Context, targetPath string) error
	Stats(ctx context.Context, targetPath string) ([]byte, error)
}

type codewireDirectVolumeOperations interface {
	IsManaged(volumeID string) (bool, error)
	Stage(volumeID, stagingTargetPath, devicePath string) error
	Publish(ctx context.Context, volumeID, targetPath string, mountInfo codewireDirectVolumeMountInfo) error
...
func (r *hostKataRuntime) command(ctx context.Context, args ...string) ([]byte, error) {
	...
	if run == nil {
		run = func(ctx context.Context, command string, args ...string) ([]byte, error) {
			return utilexec.New().CommandContext(ctx, command, args...).Output()
		}
	}
	output, err := run(ctx, nsMounterPath, commandArgs...)
	if err != nil {
		// Do not include command arguments in this error. They contain non-secret
		// mount metadata today, but keeping errors redacted prevents a future
		// metadata addition from becoming a logging disclosure.
		return nil, fmt.Errorf("host Kata direct-volume command failed")
	}
	return output, nil
}

func (r *hostKataRuntime) Add(ctx context.Context, targetPath string, mountInfo codewireDirectVolumeMountInfo) error {
...
type codewireDirectVolumeManager struct {
	mu       sync.Mutex
	stateDir string
	runtime  codewireKataRuntime
}

func newCodewireDirectVolumeManager() *codewireDirectVolumeManager {
	return &codewireDirectVolumeManager{
		stateDir: codewireDirectVolumeStateDir,
		runtime:  &hostKataRuntime{},
	}
}

func (m *codewireDirectVolumeManager) statePath(volumeID string) string {
	encoded := base64.RawURLEncoding.EncodeToString([]byte(volumeID))
	return filepath.Join(m.stateDir, encoded+".json")
}

func (m *codewireDirectVolumeManager) loadLocked(volumeID string) (*codewireDirectVolumeState, error) {
	data, err := os.ReadFile(m.statePath(volumeID))
	if err != nil {
		return nil, err
	}
	state := &codewireDirectVolumeState{}
...
func (m *codewireDirectVolumeManager) saveLocked(state *codewireDirectVolumeState) (err error) {
	...
	tmp, err := os.CreateTemp(m.stateDir, ".codewire-direct-volume-*")
	if err != nil {
...
func (m *codewireDirectVolumeManager) IsManaged(volumeID string) (bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *codewireDirectVolumeManager) Stage(volumeID, stagingTargetPath, devicePath string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	return m.saveLocked(&codewireDirectVolumeState{
		VolumeID:          volumeID,
...
func (m *codewireDirectVolumeManager) Publish(ctx context.Context, volumeID, targetPath string, mountInfo codewireDirectVolumeMountInfo) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *codewireDirectVolumeManager) Unpublish(ctx context.Context, volumeID, targetPath string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *codewireDirectVolumeManager) Unstage(ctx context.Context, volumeID string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *codewireDirectVolumeManager) Stats(ctx context.Context, volumeID, targetPath string) (*csi.NodeGetVolumeStatsResponse, error) {
	m.mu.Lock()
	state, err := m.loadLocked(volumeID)
	m.mu.Unlock()
	if err != nil {
	...
	var stats struct {
		...
		VolumeCondition *struct {
			Abnormal bool   `json:"abnormal"`
			Message  string `json:"message"`
		} `json:"volume_condition"`
	}
	...
	if stats.VolumeCondition != nil {
		response.VolumeCondition = &csi.VolumeCondition{
			Abnormal: stats.VolumeCondition.Abnormal,
			Message:  stats.VolumeCondition.Message,
		}
	}
	return response, nil
}

func codewireDirectVolumeRequested(volumeContext map[string]string) (bool, error) {
	value, exists := volumeContext[codewireKataDirectVolumeParameter]
	if !exists {
		return false, nil
	}
	if value != "true" {
		return false, status.Errorf(codes.InvalidArgument, "%s must be exactly true when present", codewireKataDirectVolumeParameter)
	}
	return true, nil
}

func (ns *NodeServer) codewireDirectVolumeMode(volumeID string, volumeContext map[string]string) (bool, error) {
	requested, err := codewireDirectVolumeRequested(volumeContext)
	if err != nil {
		return false, err
	}
	if requested {
...
func validateCodewireDirectVolume(volume *longhornclient.Volume, capability *csi.VolumeCapability) (string, error) {
	if volume == nil {
...
func (ns *NodeServer) codewireDirectVolumeMetadata(ctx context.Context, volumeContext map[string]string) (map[string]string, error) {
	...
	keyID := claim.Annotations[codewireStorageKeyIDAnnotation]
	parsed, err := uuid.Parse(keyID)
	if err != nil || parsed.String() != keyID {
		return nil, status.Error(codes.InvalidArgument, "confidential storage key id must be a canonical lowercase UUID")
	}
	return map[string]string{
		codewireStorageEncryptionKey: "luks2",
		codewireStorageSourceKey:     "auto",
		codewireStorageKeyURIKey:     codewireStorageKeyURIPrefix + keyID,
		codewireStorageFilesystemKey: "ext4",
		codewireStorageGrowKey:       "true",
	}, nil
}

func (ns *NodeServer) nodeStageCodewireDirectVolume(ctx context.Context, req *csi.NodeStageVolumeRequest, volume *longhornclient.Volume) (*csi.NodeStageVolumeResponse, error) {
	if len(req.GetSecrets()) != 0 {
		return nil, status.Error(codes.InvalidArgument, "confidential direct volumes reject CSI node secrets")
	}
	devicePath, err := validateCodewireDirectVolume(volume, req.GetVolumeCapability())
	if err != nil {
		return nil, err
	}
	...
	if _, err := ns.codewireDirectVolumeMetadata(ctx, req.GetVolumeContext()); err != nil {
		return nil, err
	}
...
func (ns *NodeServer) nodePublishCodewireDirectVolume(ctx context.Context, req *csi.NodePublishVolumeRequest, volume *longhornclient.Volume) (*csi.NodePublishVolumeResponse, error) {
	if req.GetReadonly() {
	...
	metadata, err := ns.codewireDirectVolumeMetadata(ctx, req.GetVolumeContext())
	if err != nil {
		return nil, err
	}
	...
	mountInfo := codewireDirectVolumeMountInfo{
		VolumeType: "block",
		Device:     devicePath,
